...
@Component({
  ...
  imports: [DatePipe, FormsModule, LucideAngularModule, OrganizerEventInventory],
  ...
  template: `
    <section class="management-content event-details-content">
      ...
      @else if (store.event(); as event) {
        ...
        <section class="card detail-table-card" aria-label="Setores vinculados ao evento"><div class="card-heading"><h2 class="section-title"><lucide-icon name="layers" size="19" aria-hidden="true"></lucide-icon>Setores vinculados</h2><button class="button primary" type="button" (click)="openAssociationForm()"><lucide-icon name="plus" size="17" aria-hidden="true"></lucide-icon>Associar setor</button></div>
          @if (store.sectors().length) { <div class="table-wrap"><table class="event-sector-summary-table"><thead><tr><th><span class="table-heading"><lucide-icon name="layers" size="15" aria-hidden="true"></lucide-icon>Setor</span></th><th><span class="table-heading"><lucide-icon name="circle-dollar-sign" size="15" aria-hidden="true"></lucide-icon>Inteira</span></th><th><span class="table-heading"><lucide-icon name="circle-dollar-sign" size="15" aria-hidden="true"></lucide-icon>Meia</span></th><th><span class="table-heading"><lucide-icon name="users" size="15" aria-hidden="true"></lucide-icon>Capacidade</span></th><th><span class="table-heading"><lucide-icon name="ticket" size="15" aria-hidden="true"></lucide-icon>Disponíveis</span></th><th>Ações</th></tr></thead><tbody>
            @for (sector of store.sectors(); track sector.id) { <tr><td><strong>{{ sector.name }}</strong></td><td>{{ formatPrice(sector.fullPrice) }}</td><td>{{ formatPrice(sector.halfPrice) }}</td><td>{{ formatQuantity(sector.capacity) }}</td><td>{{ formatQuantity(sector.available) }}</td><td class="sector-actions-cell"><button class="sector-remove-button" type="button" [disabled]="removingSectorId() === sector.id" (click)="requestRemoval(sector.id, sector.name)"><lucide-icon name="trash-2" size="16" aria-hidden="true"></lucide-icon>Remover</button></td></tr> }
...
export class OrganizerEventOverview implements OnInit {
  ...
  protected requestRemoval(sectorId: string, sectorName: string): void { this.sectorPendingRemoval.set({ id: sectorId, name: sectorName }); }
