...
} from '../../../core/models/event-catalog.model';
import { EventSectorInventoryResponseDTO } from '../../../core/models/inventory.model';
...
export class HttpEventApi implements EventApi {
  ...
  addSector(eventId: string, request: EventSectorMutationRequest): Observable<unknown> {
    return this.http.post<unknown>(this.apiUrl.url(`${EVENT_CATALOG_EVENTS_PATH}/${eventId}/add-sector`), request).pipe(tap(() => this.eventDetailsCache.delete(eventId)));
  }
