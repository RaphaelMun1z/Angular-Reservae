...
describe('TicketStore', () => {
  ...
  it('should list the most recent tickets first', () => {
    api.tickets = [
      { id: 'ticket-old', eventId: 'event-1', status: 'VALID', createdAt: '2026-01-10T10:00:00Z' },
      { id: 'ticket-new', eventId: 'event-2', status: 'VALID', createdAt: '2026-03-10T10:00:00Z' },
      { id: 'ticket-without-date', eventId: 'event-3', status: 'VALID' },
    ];

    store.loadTickets();

    expect(store.tickets().map((ticket) => ticket.id)).toEqual([
      'ticket-new',
      'ticket-old',
      'ticket-without-date',
    ]);
