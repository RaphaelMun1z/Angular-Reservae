...
export { TICKET_API } from '../../../core/http/contracts/tickets.contracts';
export type { TicketApi, TicketTransferRequest } from '../../../core/http/contracts/tickets.contracts';
import { TICKET_API, TicketTransferRequest } from '../../../core/http/contracts/tickets.contracts';

/* legacy contract declarations moved to core/http/contracts/tickets.contracts */
...
export class TicketStore {
  ...
  loadTickets(): void {
    ...
    this.api
      ...
      .pipe(
        tap((tickets) => {
          const normalizedTickets = tickets.map((ticket) => ({ ...ticket }));
          this._tickets.set(this.sortByMostRecent(normalizedTickets));
        }),
  ...
  private sortByMostRecent(tickets: readonly Ticket[]): Ticket[] {
    return tickets
      .map((ticket, index) => ({ ticket, index }))
      .sort((left, right) => {
        const rightTime = this.timestamp(right.ticket.createdAt);
        const leftTime = this.timestamp(left.ticket.createdAt);
